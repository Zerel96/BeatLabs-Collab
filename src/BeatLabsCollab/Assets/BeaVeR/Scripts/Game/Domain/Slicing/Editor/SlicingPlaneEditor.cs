...
namespace BeaVeR.Game.Domain.Slicing.Editor
{
  /// <remarks>
  /// Based on PlaneUsageExampleEditor from EzySliceExamples.
  /// </remarks>
  [CustomEditor(typeof(SlicingPlane))]
  public class SlicingPlaneEditor : UnityEditor.Editor
  {
    ...
    {
      SlicingPlane plane = (SlicingPlane)target;
      ...
      if (GUILayout.Button("Cut Object"))
      {
        ...
        if (!recursiveSlice)
        {
          ...
          if (hull != null)
          {
            GameObject lowerHullObject = hull.CreateLowerHull(source, crossMat);
            GameObject upperHullObject = hull.CreateUpperHull(source, crossMat);

            if (lowerHullObject != null)
            {
              lowerHullObject.transform.SetParent(source.transform.parent, worldPositionStays: false);
            }

            if (upperHullObject != null)
            {
              upperHullObject.transform.SetParent(source.transform.parent, worldPositionStays: false);
            }
     ...
     */
    public GameObject[] SliceObjectRecursive(SlicingPlane plane, GameObject obj, Material crossSectionMaterial)
    {
