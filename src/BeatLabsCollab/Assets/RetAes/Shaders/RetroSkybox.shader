Shader "RetAes/Retro Skybox" {
    Properties {
        ...
        [Space]
        [Toggle] _CubemapEnabled("Enable cubemap", Float) = 0
        [NoScaleOffset] _CubemapTex("Cubemap   (HDR)", Cube) = "black" {}
    ...
    fixed4 frag (v2f i) : COLOR {
        ...
        fixed4 c = gradient / pow(tex2D(_MainTex, i.tex), _Gain);
        
        half4 cTex = texCUBE(_CubemapTex, i.uv);
        half3 cTexHdr = DecodeHDR(cTex, _CubemapTex_HDR);
            
        return c + float4(cTexHdr, 1) * _CubemapEnabled;
    }
